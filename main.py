...
class Space():
    ...
    def hill_climb(self, maximum=None, image_prefix=None, log=False):
        ...
        self.hospitals = set(self.randomly_place_hospitals())
        ...
        while maximum is None or count < maximum:
            count += 1
            best_neighbors, best_neighbor_cost = self.find_best_neighbor()
    ...
    def randomly_place_hospitals(self):
        """Randomly places hospitals in the available space."""
        return [random.choice(list(self.available_spaces())) for _ in range(self.num_hospitals)]

    def find_best_neighbor(self):
        """Finds the best neighbor by evaluating all possible moves."""
        best_neighbors = []
        ...
        return best_neighbors, best_neighbor_cost
...
ax.scatter([j for i, j in s.hospitals], [i for i, j in s.hospitals], [z[int(i), int(j)] for i, j in s.hospitals], color='green', marker='*', label="Hospitals", s=100)

ax.set_xlabel("X-Axis")
ax.set_ylabel("Y-Axis")
ax.set_zlabel("Z-Axis (Cost)")
ax.set_title("3D Heatmap with Hospital Paths")
ax.legend()
